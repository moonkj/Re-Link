...
- [x] 높이 72px → 85px (+13px)
- [x] AppSpacing.bottomNavHeight 80 → 85 동기화
- [x] 인디케이터 top 12 → 18px (아이콘 중앙 정렬)
- [x] 사이드이펙트 전수 검사 (11개 영역, 35개 항목) — 추가 영향 0건

---

## Phase 37 — 전체 기능 QA 테스트 (2026-04-04~)

> 468개 항목 전수 QA — 실기기 테스트, Phase A~G 순차 진행

---

### Phase A: 첫 실행 (45개 항목) ✅

- [x] A01~A06: 스플래시 화면 — 정상 (로고 애니메이션, 태그라인, 자동 이동)
- [x] A07~A14: 복원 감지 화면 — 정상 (iCloud 백업 감지, 새로 시작 이동)
- [x] A15~A26: 온보딩 3단계 — 정상 (스와이프, 건너뛰기, 알림 권한)
- [x] A27~A37: 프로필 설정 — 정상 (이름 필수 검증, 사진, 생년월일, 저장)
- [x] A38~A45: 첫 가족 추가 — 정상 (노드 생성, 관계 연결, 캔버스 이동)
- [x] **수정**: 부모 아이콘 `elderly`(지팡이) → `people_outline`(두 사람)

### Phase B: 캔버스 (89개 항목) — 코드 검증 ✅

- [x] B01~B07: 기본 렌더링 — 노드, 엣지, 빈 상태, 나무 성장 OK
- [x] B08~B17: 제스처 — 팬/줌/탭/더블탭/롱프레스/드래그/드롭/겹침해소/배우자스냅 OK
- [x] B18~B23: 오버레이 — 미니맵/프롬프트/배너/스트릭/나무/뱃지 OK
- [x] B24~B30: 앱바 — 리스트토글/검색/타임라인/내보내기/센터/노드수 OK
- [x] B31~B34: 타임슬라이더 — 연도 필터, 이벤트 토스트 OK
- [x] B35~B46: 노드 추가 — 이름검증/사진/생일/고스트/플랜제한 OK
- [x] B47~B60: 노드 상세 — 4탭/온도/추모/고스트전환/나설정/액션6개 OK
- [x] B61~B73: 노드 편집 — 클리어플래그/날짜검증/키보드회피 OK
- [x] B74~B84: 관계선택/온도계 — 5관계/6단계온도/애니메이션 OK
- [x] B85~B89: 리스트뷰 — 9섹션그룹/빈상태/고스트스타일 OK
- [ ] B05~B07: DEVICE-ONLY (런타임 성능 — 실기기 확인 필요)

### Phase C: 아카이브/기억 (61개 항목) — 코드 검증 ✅

- [x] C01~C10: 아카이브 5탭 필터/정렬/검색 OK
- [x] C11~C20: 노드별 기억 — 타입필터/플랜제한/프라이버시 OK
- [x] C21~C40: 기억 추가 — 사진/음성/메모/영상 CRUD, 녹음 플로우, 태그 OK
- [x] C41~C50: 기억 상세 — PathUtils 경로해석, 플레이어, 프라이버시 토글 OK
- [x] C51~C61: 스토리피드/검색 — 날짜순정렬, 디바운스400ms OK

### Phase D: 가족 허브 (53개 항목) — 코드 검증 ✅

- [x] D01~D15: 허브/생일/온도 — 달력/윤년/그래프/삭제확인 OK
- [x] D16~D25: 레시피 — CRUD/공유(sharePositionOrigin)/PathUtils OK
- [x] D26~D33: 가족지도 — 자동완성/핀/타임라인 OK
- [x] D34~D38: 부케리포트 — 노드이름(UUID아님)/꽃타입/요약 OK
- [x] D39~D53: 초대/합류/멤버 — 서버폴백/딥링크코드/로컬코드 OK
- [x] **수정**: D42 초대 Share sharePositionOrigin UI에서 전달

### Phase E: 탐색 허브 (83개 항목) — 코드 검증 ✅

- [x] E01~E15: 캡슐 — 생성/봉인/열기(3초안전)/삭제(롱프레스)/알림취소 OK
- [x] E16~E28: 보이스유언 — 녹음/경로(toRelative저장/toAbsolute재생)/삭제/알림취소 OK
- [x] E29~E35: 뱃지 — 20개/4등급/3열그리드/진행도 OK
- [x] E36~E42: Wrapped — 6슬라이드/빈이름가드/sharePositionOrigin OK
- [x] E43~E50: 성씨탐색 — 씨접미사제거/1자성씨/2자본관/확장카드 OK
- [x] E51~E63: ThenNow/스냅샷 — safeCast/shareOrigin/4스타일 OK
- [x] E64~E83: 아트카드/족보저장/Heritage/추모/의례가이드 OK

### Phase F: 설정 (78개 항목) — 코드 검증 ✅

- [x] F01~F10: 설정화면 — 데이터관리라벨/보관정책안내/카카오계정/프로필동기화 OK
- [x] F11~F16: 로그인 — Apple/Google/Kakao EnvConfig/취소처리 OK
- [x] F17~F30: 구독/백업 — 4섹션라벨/플랜게이팅/자동백업/WAL OK
- [x] F31~F78: 접근성/개인정보/피드백/관리자 — 전체 정상

### Phase G: 공통 (59개 항목) — 코드 검증 ✅

- [x] G01~G06: 딥링크/백업 — code→joinFamily/token→acceptInvite/WAL/ASCII파일명 OK
- [x] G07~G13: 플랜제한 — CLAUDE.md 스펙 일치 (15/50/5분/20GB/6명/100GB/999명) OK
- [x] G14~G17: 다크/라이트 — static get 타이포/brightness-aware 컬러/앱 리빌드 OK
- [x] G18~G59: 백업복원/미디어경로/초대플로우/광고게이팅 OK
...
| 총 검증 항목 | **468개** |
| PASS | **465개** |
| FAIL → 수정 완료 | **0개** (D42 수정 완료) |
| DEVICE-ONLY | **3개** (런타임 성능) |

### 백업/공유/복원 심층 검증 ✅

- [x] 백업 생성: WAL체크포인트→DB복사→미디어→매니페스트→ZIP 순서 정상
- [x] 4개 복원 경로: 파일가져오기/클라우드/첫실행/외부.rlink 전부 provider 무효화 정상
- [x] 복원 실패 롤백: DB 롤백 + 미디어 롤백 + temp 정리 정상
- [x] .rlink 병합: 트랜잭션 래핑, 엣지/기억/미디어 병합 정상
- [x] 클라우드 동기화: Pull(서버→로컬)/Push(로컬→서버) 정상, 업로드 큐 동시성 3개 제한
